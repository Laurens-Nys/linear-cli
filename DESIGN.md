...
|---|---|---|
| `issue list` | filters: `--mine`, `--assignee <name>`, `--unassigned`, `--state <name>`, `--label <name>` (repeat = AND), `--project`, `--cycle current\|next\|previous\|<n>`, `--parent <id>`, `--updated-since <ISO date>`, `--created-since`, `--archived`, `--sort updated\|created\|priority\|manual` (default updated) | rows `{id,title,state,assignee,priority,updated}`; with `--mine` drop assignee |
...
| Command | Behavior | Output |
|---|---|---|
| `tui` | read-only browser for my assigned issues; All/Started/Todo/Done view tabs plus session-only Team, Project, and Sort dropdowns; All stays Mine + Open, Done is completed assigned work; Linear state names group the list while the selected server sort remains intact inside groups; the detail pane renders Linear markdown through OpenTUI `MarkdownRenderable`, with `beautiful-mermaid` ASCII for `mermaid` fences; panes are transparent to the terminal background; mouse-first chrome; the footer lists only hidden keys (`/`, `r`, `q`, and `esc` after Enter); j/k and wheel browse the list without changing the open issue; click or Enter opens it, Escape returns to the list; `/` searches titles; filters, view, and layout are session-only; requires interactive stdin and stdout; the compiled binary copies OpenTUI's native library out of Bun's embedded filesystem into `/tmp` before `dlopen`, so Infisical's agent-proxy sandbox can load it | full-screen terminal interface |
...
src/
  ...
  tui/           OpenTUI browser: app, issue list, markdown+mermaid detail, data, theme, native extract, run
